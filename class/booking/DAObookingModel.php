...
require dirname(__FILE__).'/../DAOModel.php';
...
class BookingDAO extends DAO
{
	...
	public function bookingCustomerQuery($lastName, $dateOfBirth, $address1, $phoneNumber)
	{
		...
		if ($result->num_rows > 0) 
		{
			...
			echo "<form action= ".htmlspecialchars($_SERVER["PHP_SELF"])."?action=bookingCreate autocomplete='on' method='post'>";
	...
	public function bookingSearchCustomerQuery($lastName, $dateOfBirth, $address1, $phoneNumber)
	{
		...
		{
			echo "<span class='notice'>* Search displays first 10 Results</span>";
			echo "<form action= ".htmlspecialchars($_SERVER["PHP_SELF"])."?action=bookingSearch autocomplete='on' method='post'>";
			echo "<h1 style='color:#16e059;'>Results:</h1>";
			echo "<table border='1' style='width:100%'>
				<tr>
